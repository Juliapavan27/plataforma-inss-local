      ...
      <main className="relative">
        ...
        <section className="relative isolate overflow-hidden">
          <div className="pointer-events-none absolute inset-0 -z-10">
            ...
            <div className="absolute -right-32 top-10 h-[380px] w-[380px] rounded-full bg-gold-200/40 blur-3xl" />
          </div>
          <div className="container py-16 md:py-20">
            <div className="mx-auto max-w-2xl text-center">
              ...
              <h1 className="mt-5 font-display text-4xl font-bold text-balance text-ink-950 md:text-5xl">
                Entenda seus direitos{" "}
                <span className="italic text-gradient-brand">em linguagem simples.</span>
              </h1>
              <p className="mx-auto mt-4 max-w-xl text-lg leading-relaxed text-ink-600 text-pretty">
        ...
        <section className="container pb-24">
          ...
          <Link
            ...
            className="group relative block overflow-hidden rounded-3xl border border-ink-200/70 bg-ink-950 p-8 text-white shadow-lift transition hover:-translate-y-1 md:p-12"
          >
          ...
          <div className="mt-6 grid gap-6 md:grid-cols-3">
            {demais.map((g) => (
              <Link
                key={g.slug}
                href={`/guias/${g.slug}`}
                className="group flex flex-col rounded-2xl border border-ink-200/70 bg-white/80 p-6 shadow-ring backdrop-blur-sm transition hover:-translate-y-1 hover:shadow-lift"
              >
                <div className="flex flex-wrap items-center gap-2 text-[11px] font-semibold uppercase tracking-[0.14em]">
                  <span className="rounded-full bg-brand-50 px-2.5 py-1 text-brand-700 ring-1 ring-brand-200/60">
                    {g.category}
                  </span>
                ...
                <h3 className="mt-4 font-display text-xl font-semibold leading-snug text-balance text-ink-950">
                  {g.title}
                </h3>
          ...
          {/* CTA */}
          <div className="mt-16 rounded-3xl border border-brand-200/60 bg-gradient-to-br from-brand-50 to-white p-8 text-center md:p-12">
            <h2 className="font-display text-2xl font-semibold text-balance text-ink-950 md:text-3xl">
              Já sabe o motivo da sua negativa?
            </h2>
            <p className="mx-auto mt-3 max-w-lg text-ink-600 text-pretty">
              Monte seu recurso administrativo com fundamentação técnica e receba em PDF e
              Word, pronto para protocolar no Meu INSS.
            </p>
            <Link href="/novo-recurso" className="btn-primary mt-7 px-7 py-4 text-base">
